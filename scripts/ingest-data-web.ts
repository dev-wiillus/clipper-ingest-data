...
import download from '@/utils/download-web';
...
export const run = async () => {
  try {
    // const date = new Date('October 19, 2022');
    // console.log(date);

    // pdf 경로에서 파일 다운로드하기
    const downloadPath = '/home/changmin/다운로드/pdf-test';
    const url = 'https://www.mckinsey.com/kr/our-insights/our-insights';
    const fileName = await download(downloadPath, url);

    //  media 테이블에서 web url 가져오기
